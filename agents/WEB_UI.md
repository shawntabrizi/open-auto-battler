...
## Contract Client
- `web/src/contract/index.ts` exposes `createContractBackend`, the only place that talks to the chain.
  It uses `@dotdm/cdm`'s `createCdm()` over a PAPI client and signs via
  `@parity/product-sdk-signer`'s `SignerManager` (`DevProvider` for local zombienet, `HostProvider` when running inside a Polkadot Triangle host).
- Typed contract bindings live in `web/.cdm/cdm.d.ts` (generated by `cdm install`)
  and the manifest in `web/cdm.json`. There is no hand-rolled ABI module.
- `BattleReported` events are extracted from the substrate `Revive.ContractEmitted`
  events surfaced in `TxResult.events`.
