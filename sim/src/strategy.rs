...
fn trigger_multiplier(trigger: &AbilityTrigger) -> i32 {
    match trigger {
        ...
        AbilityTrigger::OnBuy | AbilityTrigger::OnSell | AbilityTrigger::OnShopStart => 0,
    }
}
...
fn classify_role(card: &UnitCard) -> BoardRole {
    ...
    let has_repeating_support = card.abilities.iter().any(|a| {
        matches!(
            a.trigger,
        ...
        .map(|i| {
            ...
            state
                ...
                .map(|c| {
                    ...
                    (c.economy.play_cost, c.economy.pitch_value, score)
                })
                .unwrap_or((0, 0, 0))
        })
        .collect();
