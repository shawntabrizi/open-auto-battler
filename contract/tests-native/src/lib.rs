...
mod tests {
    ...
    use oab_game::{GamePhase, GameSession};
    use parity_scale_codec::{Decode, Encode};
    use std::collections::BTreeMap;

    // ── Types mirroring the contract ─────────────────────────────────────────────

    const PHASE_SHOP: GamePhase = GamePhase::Shop;
    const PHASE_COMPLETED: GamePhase = GamePhase::Completed;
    ...
    impl CardRegistry {
        ...
        fn register_all(all_cards: &[UnitCard], all_sets: &[CardSet]) -> Self {
            ...
            for card in all_cards {
                reg.cards.insert(card.id, card.clone());
            }
            for (i, set) in all_sets.iter().enumerate() {
                reg.sets.insert(i as SetIdValue, set.clone());
            }
            reg
        }

        fn load_card(&self, id: CardId) -> Option<&UnitCard> {
            self.cards.get(&id)
        }
        fn load_set(&self, id: SetIdValue) -> Option<&CardSet> {
            self.sets.get(&id)
        }
    ...
    fn start_game(registry: &CardRegistry, set_id: SetIdValue, seed: u64) -> ArenaSession {
        ...
        session.hand = shop.hand;
        session.board = shop.board;
    ...
    #[test]
    fn individual_cards_fit_storage() {
    ...
    #[test]
    fn session_fits_storage_initial() {
        let reg = setup_registry();
    ...
    #[test]
    fn session_fits_storage_full_board() {
        let reg = setup_registry();
    ...
    fn session_fits_at_every_bag_size() {
        for n in 0..=50u8 {
            let s = ArenaSession {
                bag: vec![CardId(10); n as usize],
                hand: vec![CardId(10); 5.min(50 - n as usize)],
                board: vec![
                    Some(BoardUnit {
                        card_id: CardId(10),
                        perm_attack: 99,
    ...
    fn bag_correct_size() {
        ...
        assert_eq!(create_starting_bag(cs, 42, 50).len(), 50);
    }
    #[test]
    fn bag_deterministic() {
        let cs = &sets::get_all()[0];
        assert_eq!(
            create_starting_bag(cs, 42, 50),
            create_starting_bag(cs, 42, 50)
        );
    }
    #[test]
    fn bag_varies_with_seed() {
        ...
        assert_ne!(
            create_starting_bag(cs, 42, 50),
            create_starting_bag(cs, 43, 50)
        );
    }
    #[test]
    fn bag_only_draftable() {
        let cs = &sets::get_all()[0];
        let ok: Vec<CardId> = cs
            .cards
            .iter()
            .filter(|e| e.rarity > 0)
            .map(|e| e.card_id)
            .collect();
        for id in create_starting_bag(cs, 42, 50) {
            assert!(ok.contains(&id));
        }
    }
    #[test]
    fn bag_matches_pallet() {
        let cs = &sets::get_all()[0];
        for seed in [1, 42, 999, u64::MAX] {
            assert_eq!(
                create_starting_bag(cs, seed, 50),
                oab_game::sealed::create_starting_bag(cs, seed, 50),
                "seed {seed}"
            );
        }
    }
    ...
    #[test]
    fn hand_correct_size() {
        let reg = setup_registry();
        let s = start_game(&reg, 0, 42);
        assert_eq!(s.hand.len(), 5);
        assert_eq!(s.bag.len(), 45);
    }
    #[test]
    fn hand_deterministic() {
        let reg = setup_registry();
        assert_eq!(start_game(&reg, 0, 42).hand, start_game(&reg, 0, 42).hand);
    }
    #[test]
    fn hand_preserves_total() {
        let reg = setup_registry();
        let s = start_game(&reg, 0, 42);
        assert_eq!(s.hand.len() + s.bag.len(), 50);
    }
    #[test]
    fn hand_empty_bag() {
        let mut s = ArenaSession {
            bag: vec![],
            hand: vec![],
            board: vec![None; 5],
            mana_limit: 3,
    ...
    fn start_game_initial_state() {
        ...
        let s = start_game(&reg, 0, 42);
        assert_eq!(
            (s.round, s.lives, s.wins, s.mana_limit, s.phase, s.set_id),
            (1, 3, 0, 3, PHASE_SHOP, 0)
        );
        assert_eq!(s.hand.len(), 5);
        assert_eq!(s.bag.len(), 45);
    ...
    #[test]
    fn start_game_different_seeds() {
        let reg = setup_registry();
        assert_ne!(start_game(&reg, 0, 1).hand, start_game(&reg, 0, 2).hand);
    }
    ...
    fn mana_progression() {
        ...
        assert_eq!(
            [
                c.mana_limit_for_round(1),
                c.mana_limit_for_round(2),
                c.mana_limit_for_round(5),
                c.mana_limit_for_round(8),
                c.mana_limit_for_round(100)
            ],
            [3, 4, 7, 10, 10]
        );
    }
    ...
    fn make_shop(s: &ArenaSession, reg: &CardRegistry) -> ShopState {
        ...
        ShopState {
            ...
            hand: s.hand.clone(),
            board: s.board.clone(),
            mana_limit: s.mana_limit,
            shop_mana: s.shop_mana,
            round: s.round,
    ...
    fn empty_turn_valid() {
        let reg = setup_registry();
        let s = start_game(&reg, 0, 42);
        let mut shop = make_shop(&s, &reg);
        assert!(verify_and_apply_turn(&mut shop, &CommitTurnAction { actions: vec![] }).is_ok());
    }
    #[test]
    fn burn_gives_mana() {
        let reg = setup_registry();
        ...
        .unwrap();
        assert!(shop.shop_mana > 0);
    }
    #[test]
    fn invalid_hand_index() {
        let reg = setup_registry();
        let s = start_game(&reg, 0, 42);
        let mut shop = make_shop(&s, &reg);
        assert!(verify_and_apply_turn(
            &mut shop,
            &CommitTurnAction {
    ...
    #[test]
    fn turn_advances_round() {
        let reg = setup_registry();
        let mut s = start_game(&reg, 0, 42);
        submit_turn(
            &mut s,
            &reg,
            &CommitTurnAction { actions: vec![] },
            make_weak_enemy(),
        );
        if s.phase == PHASE_SHOP {
            assert_eq!(s.round, 2);
            assert_eq!(s.mana_limit, 4);
            assert_eq!(s.hand.len(), 5);
        }
    }
    ...
    fn game_ends_zero_lives() {
        ...
        {
            assert_eq!((s.lives, s.phase), (0, PHASE_COMPLETED));
        }
    }

    #[test]
    fn game_ends_ten_wins() {
        let reg = setup_registry();
        let mut s = start_game(&reg, 0, 42);
        s.wins = 9;
        s.board[0] = Some(BoardUnit {
        ...
        {
            assert_eq!((s.wins, s.phase), (10, PHASE_COMPLETED));
        }
    }
    ...
    fn battle_deterministic() {
        let p = vec![CombatUnit::from_card(UnitCard::new(
            CardId(10),
            "A",
            3,
            5,
        ...
        let e = vec![CombatUnit::from_card(UnitCard::new(
            CardId(20),
            "B",
            2,
            4,
    ...
    #[test]
    fn combat_unit_abilities_round_trip() {
        ...
        assert_eq!(
            CombatUnit::decode(&mut &CombatUnit::from_card(card).encode()[..])
                .unwrap()
                .abilities
                .len(),
            1
    ...
    #[test]
    fn ghost_from_full_board() {
        let board: Vec<Option<BoardUnit>> = (0..5)
            .map(|i| {
                Some(BoardUnit {
                    card_id: CardId(10 + i),
    ...
    #[test]
    fn ghost_preserves_permanent_stats() {
        let board = vec![Some(BoardUnit {
            card_id: CardId(10),
            perm_attack: 99,
            perm_health: -50,
        })];
    ...
    impl GhostPoolStore {
        ...
        fn load(
            ...
            wins: RoundValue,
            lives: RoundValue,
        ) -> Vec<Vec<GhostBoardUnit>> {
            self.pools
                .get(&(set_id, round, wins, lives))
                .cloned()
                .unwrap_or_default()
        }
    ...
    #[test]
    fn ghost_pool_push_empty_is_noop() {
        let mut store = GhostPoolStore::new();
    ...
    #[test]
    fn ghost_pool_brackets_are_independent() {
        let mut store = GhostPoolStore::new();
        let ghost_a = vec![GhostBoardUnit {
            card_id: CardId(1),
            perm_attack: 0,
            perm_health: 0,
        }];
        let ghost_b = vec![GhostBoardUnit {
            card_id: CardId(2),
            perm_attack: 0,
            perm_health: 0,
        }];
    ...
    #[test]
    fn ghost_pool_encoding_round_trip() {
    ...
    #[test]
    fn ghost_pool_fits_storage() {
        // Max pool: 10 ghosts, each with 5 units (6 bytes each) + SCALE overhead
        let full_pool: Vec<Vec<GhostBoardUnit>> = (0..10)
            .map(|_| {
                (0..5)
                    .map(|i| GhostBoardUnit {
                        card_id: CardId(i),
                        perm_attack: 99,
                        perm_health: 99,
                    })
    ...
    fn select_is_deterministic() {
        ...
        for i in 0..5u16 {
            store.push(
                ...
                3,
                vec![GhostBoardUnit {
                    card_id: CardId(card_set.cards[i as usize].card_id.0),
                    perm_attack: 0,
                    perm_health: 0,
                }],
            );
    ...
    #[test]
    fn select_applies_permanent_stats() {
        let reg = setup_registry();
        let card_set = reg.load_set(0).unwrap();
        let card_pool = reg.build_card_pool(card_set);
    ...
    #[test]
    fn end_game_archives_final_board() {
        let reg = setup_registry();
        ...
        session.board[2] = Some(BoardUnit {
            card_id: CardId(20),
            perm_attack: 0,
            perm_health: 0,
        });
    ...
    #[test]
    fn end_game_empty_board_no_ghost() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
    ...
    #[test]
    fn end_game_uses_correct_bracket() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
        session.board[0] = Some(BoardUnit {
            card_id: CardId(10),
    ...
    #[test]
    fn end_game_after_full_victory() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
    ...
    #[test]
    fn end_game_after_final_defeat() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
        session.lives = 1;
        let result = submit_turn(
    ...
    #[test]
    fn abandon_does_not_archive_ghost() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
        session.board[0] = Some(BoardUnit {
            card_id: CardId(10),
        ...
        assert!(ghost_store
            .load(0, session.round, session.wins, session.lives)
            .is_empty());
    }

    #[test]
    fn abandon_completed_game_no_ghost() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
        session.board[0] = Some(BoardUnit {
            card_id: CardId(10),
    ...
    #[test]
    fn submit_turn_archives_ghost_before_battle() {
    ...
    #[test]
    fn end_game_ghost_selectable_by_next_player() {
        let reg = setup_registry();
        let card_set = reg.load_set(0).unwrap();
        let card_pool = reg.build_card_pool(card_set);
        let mut ghost_store = GhostPoolStore::new();
        ...
        let base_health = card_pool
            .get(&card_set.cards[0].card_id)
            .unwrap()
            .stats
            .health;
    ...
    #[test]
    fn permanent_stats_persist_across_rounds() {
        let reg = setup_registry();
        let mut session = start_game(&reg, 0, 42);
    ...
    #[test]
    fn completed_session_encodes_correctly() {
        let session = ArenaSession {
            bag: vec![],
            hand: vec![CardId(5)],
            board: vec![
    ...
    #[test]
    fn completed_session_decodes_as_shared_game_session() {
        let session = ArenaSession {
            bag: vec![],
            hand: vec![CardId(5)],
            board: vec![
        ...
        let mut encoded = session.encode();
        encoded.extend_from_slice(&oab_game::sealed::default_config().encode());

        let decoded = GameSession::decode(&mut &encoded[..]).unwrap();
        assert_eq!(decoded.state.phase, GamePhase::Completed);
        assert_eq!(decoded.state.wins, 10);
    }

    #[test]
    fn ghost_board_unit_encoding_matches_types_crate() {
        // Verify GhostBoardUnit encoding is the same between test and battle crate
        let unit = GhostBoardUnit {
            card_id: CardId(42),
            perm_attack: -3,
            perm_health: 10,
        };
