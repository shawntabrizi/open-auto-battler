...
export const useMultiplayerStore = create<MultiplayerState>((set, get) => ({
    ...
    return new Promise((resolve, reject) => {
      ...
      peer.on('open', (id) => {
        set({
          ...
          logs: [
            ...get().logs,
            { timestamp: Date.now(), message: `Peer initialized with ID: ${id}` },
          ],
        });
        resolve(id);
      });
      ...
      peer.on('error', (err) => {
        set({
          logs: [...get().logs, { timestamp: Date.now(), message: `Peer error: ${err.message}` }],
        });
        reject(err);
      });
      ...
      peer.on('connection', (conn) => {
        set({
          logs: [
            ...get().logs,
            { timestamp: Date.now(), message: `Incoming connection from ${conn.peer}` },
          ],
          isHost: true,
        });
  ...
  connectToPeer: (peerId: string) => {
    ...
    set({
      status: 'connecting',
      isHost: false,
      logs: [...get().logs, { timestamp: Date.now(), message: `Connecting to ${peerId}...` }],
    });
  ...
  setConnection: (conn: DataConnection) => {
    ...
    conn.on('error', (err) => {
      set({
        logs: [
          ...get().logs,
          { timestamp: Date.now(), message: `Connection error: ${err.message}` },
        ],
      });
    });
  },
